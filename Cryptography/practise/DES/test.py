...
def encrypt(msg, key):
    ...
    round_key = []
    for i in range(16):
        l = leftRotate(l, shift_table[i])
        r = leftRotate(r, shift_table[i])
        combined = l + r
        roundKey = permute(combined, perm_cho_2, 48)
        round_key.append(roundKey)

    for i in range(16):
        right_expand = permute(right, expan_perm, 48)
        xor_x = xor(right_expand, round_key[i])
        s_box_str = ""
        for j in range(8):
            ...
            row = bin_to_dec(row)
            col = bin_to_dec(col)
            s_box_str += dec_to_bin(s_box[j][row][col])
        result = permute(s_box_str,perm_table,32)
        left = xor(result,left)
        if i!=15:
            right,left=left,right
    combined = left+right
    cipher = permute(combined,final_perm,64)
    return cipher
...
# key = input("Enter Key:")
msg = "768908ABCDEF1234"
key = "123456789ABCDEF0"
cipher=(encrypt(msg,key))
print(bin_to_hexa(cipher))
pt = decrypt(bin_to_hexa(cipher),key)
print(bin_to_hexa(pt))